...
We start by loading the control book for 2002 (M X F L S - Book C). Within the book there are four pertinent data sets to be used:
...
Some very interesting results were observed in the first PCA analysis for continuous variables:
...
We chose the CART model to obtain information about the predictive accuracy for the variable `attend_school` of each covariate. We conjectured that the top 6 covariates were enough controls to include in our linear regression\footnote{For more details see Appendix - Part 4 - Covariate Importance (CART model)}; `age` age of the child at time of interview, `hours_sleep` average sleeping hours for child, `hh_avg_educ` average level of education of household members, `hh_income` household combined income, `cult_act` whether ther have performed recent cultural activites &`domestic_hwork` whether the child helps in domestic household work.
