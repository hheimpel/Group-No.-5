...
Since the data sets were in `dta` form, the `haven` package was required to load them into the environment and the package `here` was used was well for reproducibility purposes. Furthermore, use of the `tidyverse` package was used extensively throughout the process.
...
We then proceeded to load individual level data for children by loading the book V for 2002 (M X F L S - Book V). Within the book there are six pertinent data sets to be used:
...
These data sets contained many variables with excessive amounts of missing values. Once again we went through all data sets to identify variables which had excesssive amounts of missing values.
```{r, cache=TRUE}
...
```{r, fig.align="center", fig.width=8, fig.height=4, cache=TRUE, include=TRUE}
master_2002_num <- master_2002 %>%
  select_if(is.numeric) %>%
...
```{r, include=TRUE, fig.align="center", fig.width=8, fig.height=4, cache=TRUE}
mod_list <- list("CART" = mod_cart, "Forest" = mod_forest)

dotplot(resamples(mod_list))
```

We chose the CART model to obtain information about the predictive accuracy for the variable `attend_school` of each covariate. We conjectured that the top 6 covariates were enough controls to include in our linear regression (Appendix - Part 3), these are:
